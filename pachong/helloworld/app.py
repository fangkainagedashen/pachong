...
global probedata
probedata = {}
probedata['data'] = probe_list
probedata['version'] = '1.0'
	...
	#		wr.write('<pre><h3 style="color:blue">local_time		||url			||http_code||probe_time</h3></pre>')
		...
		with open(path,'w') as output:
			...
			for url in range (0,len(list)):
				...
				try:
					tuple = get_status(list[url])
				except:
					output.write('<span style="color:blue">'+'can not connect '+ list[url]+'</span>'+'\n')
					dic['local_time'] = str(i)
					dic['url'] = list[url]
					dic['http_code'] = '0'
					dic['probe_time'] = '0.00'
					dic['main'] = '1'
					probe_list.append(dic)
				...
				for link_url in link:
					...
					if href:
						...
						try:
							...
							output.write(str(i)+' can not connect '+ href+'\n')
							dic['local_time'] = str(i)
							dic['url'] = href
				...
				#			print 'link=',link
							try:
								...
								output.write(str(i)+' can not connect '+ link+'\n')
								dic['local_time'] = str(i)
								dic['url'] = link
								dic['http_code'] = str(data[1])
								dic['probe_time'] = str(data[2])
							...
							total_time += float(data[2])
						#	print 'total_time=',total_time
					elif 'lazy' in str(picture):
						link = picture.get('lazy')
						if link:
							if 'http:' not in link:
								#link = list[url]+link
								pass
				#			print 'link=',link
							...
							dic = {}
							total_time += float(data[2])
					elif '_src' in str(picture):
						link = picture.get('_src')
						if link:
							if 'http:' not in link:
								#link = list[url]+link
								pass
				...
				#			print 'link=',link
							...
							dic = {}
							total_time += float(data[2])
					#elif 'data-src' in str(picture) and 'www.taobao.com' not in list[url]:
					elif 'data-src' in str(picture) :
						#if 'taobao' not in list[url] :
				...
				#	print 'link=',link
					if link:
						try:
							...
							output.write(str(i)+' can not connect '+ str(link) +'\n')
							dic['local_time'] = str(i)
							dic['url'] = data[0]
							dic['http_code'] = str(data[1])
							dic['probe_time'] = str(data[2])
				...
				for src in span:
					link = src.get('data-adparam-cupid')
				#	print 'link=',link
					if link:
						try:
							data = get_status(link)
						...
						probe_list.append(dic)
						dic = {}
					link = src.get('data-adparam-cupid_ares')
				#	print 'link=',link
					if link:
						try:
							data = get_status(link)
				...
				#	link = src.get('style')
				##	print 'link=',link
				#	link = link.split(';')[0].split('url')[1][1:-1]
				#	if link:
				#		try:
				#			data = get_status(link)
				#		except:
